...
problems. It is part of the supplementary material associated to a bunch of
TowardsDataScience blog posts on the subject [1, 2] as well as of an
introductory course to deep learning for beginners.

[1] Link to the TDS post on Rosenblatt's perceptron : https://tinyurl.com/y2ccfyrc
[2] Link to the Adaptive Linear Units :
...
def main(cmap="coolwarm"):
    ...
    plt.savefig(
        "imgs/one_misclassified_dataset.png",
        bbox_inches="tight",
        transparent=True,
        # dpi=300,
    )
    ...
    ax.set_title(r"Misclassified points : %.2d" %model.errors_[-1])
    ...
    p, = ax.plot(x, decision(x), color="k")
    ...
    def animate(i):
        model.fit(X, y, maxiter=i)
        def decision(x): return -(model.weights[0] * x + model.bias)/model.weights[1]
        p.set_data(x, decision(x))
        ax.set_title(r"Misclassified points : %.2d" %model.errors_[-1])
        return p,
